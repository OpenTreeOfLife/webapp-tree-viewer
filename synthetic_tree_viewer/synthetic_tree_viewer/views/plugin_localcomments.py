...
def index(request):
    ...
    url = (
        request.POST.get("url", None)  # ideal case, unambiguous
    ...
    filter = (
        request.params.get("filter", None) or request.environ.get("filter", None) or ""
    )
    ...
    def node(comment):
        ...
        if comment.get("comments") and comment.get("comments") > 0:
            ...
            try:
                ...
                try:
                    ...
                    sys.stderr.write(
                        "Error: got a {c} from {u}\n".format(
                            c=resp.status_code, u=get_children_url
                        )
                    )
                except:
        ...
        author_display_name, author_link = extract_best_author_details(
        ...
        try:  # TODO: if not comment.deleted:
            ...
            has_supporting_reference_url = supporting_reference_url and (
                supporting_reference_url != "None"
            )
            ...
            markup = LI(
                DIV(  ##T('posted by %(first_name)s %(last_name)s',comment.created_by),
                    ...
                    and DIV(
                        comment["title"],
                        A(T("on GitHub"), _href=comment["html_url"], _target="_blank"),
                        _class="topic-title",
                    )
                    or "",
                    DIV(
                        safe_comment_markup,
                        _class=(
                            issue_node and "body issue-body" or "body comment-body"
                        ),
                    ),
                    (
                        DIV(
                            A(
                                T("Supporting reference (opens in a new window)"),
                                _href=supporting_reference_url,
                                _target="_blank",
                            ),
                            _class="body issue-supporting-reference",
                        )
                        if has_supporting_reference_url
                        else ""
                    ),
                    DIV(
                        ...
                        (
                            SPAN(
                                " ", metadata.get("Feedback type"), " ", _class="badge"
                            )
                            if metadata.get("Feedback type")
                            else ""
                        ),
                        T(
                            " - %s",
                            pretty_date(
                                utc_to_local(
                                    datetime.strptime(
                                        comment["created_at"], GH_DATETIME_FORMAT
                                    )
                                ),
                                T,
                            ),
                        ...
                        SPAN(
                            ...
                            or "",
                            issue_node
                            and comment["user"]["login"] == current_user_id
                            and SPAN(" | ")
                            or "",
                            (
                ...
                issue_node
                and DIV(
                    _class="reply", _style=("" if child_comments else "display: none;")
                )
                or "",
    ...
    if thread_parent_id == "delete":
        ...
        if user_is_logged_in(request):
            author_link = "[{0}]({1})".format(
                auth_user["display_name"], auth_user["github_profile_url"]
            )
        ...
        if thread_parent_id == "0":
            ...
            if url.startswith("http"):
                ...
                url_link = "[{0}]({1}{2})".format(
                    url, request.environ.get("HTTP_ORIGIN", "UNKNOWN_ORIGIN"), url
                )

            # add full metadata for an issue
            footer = build_comment_metadata_footer(
                comment_type="starter",
                metadata={
                    "Author": author_link,
                    "Upvotes": 0,
                    "URL": url_link,
    ...
    if filter == "skip_comments":
        ...
        log.debug(
            ">>> no match for filter '{}'... using default comment filter (url?): ".format(
                filter
            )
        )
...
def update_github_headers(request):
    ...
    GH_GET_HEADERS = {
        "Authorization": get_auth_header_value(request),
...
def add_or_update_comment(msg_data, comment_id=None, parent_issue_id=None):
    ...
    if comment_id:
        ...
        url = "{0}/repos/OpenTreeOfLife/feedback/issues/comments/{1}".format(
            GH_BASE_URL, comment_id
        )
        resp = requests.patch(url, headers=GH_POST_HEADERS, data=json.dumps(msg_data))
    else:
        # create a new comment
        url = "{0}/repos/OpenTreeOfLife/feedback/issues/{1}/comments".format(
            GH_BASE_URL, parent_issue_id
        )
        resp = requests.post(url, headers=GH_POST_HEADERS, data=json.dumps(msg_data))
    resp.raise_for_status()
    try:
...
def close_issue(issue_id):
    ...
    resp = requests.patch(
        url, headers=GH_POST_HEADERS, data=json.dumps({"state": "closed"})
    )
    resp.raise_for_status()
    try:
        resp_json = resp.json()
...
def delete_comment(comment_id):
    ...
    resp = requests.delete(url, headers=GH_GET_HEADERS)
    resp.raise_for_status()
    try:
        resp_json = resp.json()
    except:
...
def clear_local_comments(request):
    ...
    if local_url or local_ott_id or local_synth_node_id:
        ...
        if local_url:
            ...
            # print(root_relative_url)
            clear_matching_cache_keys(
                "^localcomments:.*'url': '%s'.*" % root_relative_url
            )
        if local_ott_id:
            clear_matching_cache_keys(
                "^localcomments:.*'ottol_id': '%s'.*" % local_ott_id
            )
        if local_synth_node_id:
            clear_matching_cache_keys(
                "^localcomments:.*'synthtree_node_id': '%s'.*" % local_synth_node_id
            )
    else:
...
def extract_safe_html_comment(comment):
    ...
    link_regex = re.compile(
        r"""(?x)( # verbose identify URLs within text
