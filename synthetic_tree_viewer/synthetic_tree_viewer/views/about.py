...
common_version_notes_attributes = {
    "table": ["class"],
}
...
def fetch_current_synthesis_source_data(request):
    ...
    try:
        ...
        # as usual, this needs to be a POST (pass empty fetch_args)
        study_metadata_response = requests.post(
            fetch_url, data=json.dumps({"verbose": True}), headers=json_headers
        )
...
def progress(request):
    ...
    if len(sorted_dates) == 0:
        warnings.add(
            "No phylesystem data was found! Most stats below are probably incomplete."
        )
    else:
        ...
        monthly_milestones = list(
            rrule.rrule(rrule.MONTHLY, dtstart=first_date, until=last_date)
        )
        ...
        def nearest_phylesystem_datestring(target_date):
            ...
            if abs(target_date - prior_timestamp) <= abs(
                target_date - following_timestamp
            ):
        ...
        monthly_milestones = [
            nearest_phylesystem_datestring(m) for m in monthly_milestones
        ]
    ...
    for date in sorted(dates, reverse=False):
        ...
        if synth_ott_version:
            ...
            if ott_synth_version_info is None:
                warnings.add(
                    "specified version {v} of OTT not found!".format(
                        v=synth_ott_version
                    )
                )
        else:
            if synth_v:
                ...
                warnings.add(
                    "No specified version of OTT for some synthesis releases; guessing OTT versions based on synth-date!"
                )
            else:
            ...
            if ott_synth_version_info is None:
                warnings.add(
                    "No version of OTT found on-or-before date {d}!".format(d=date)
                )
        ...
        if ott_synth_version_info is None:
            ...
            if ott_synth_version_info is None:
                warnings.add(
                    "OTT info for version {v} not found!".format(
                        v=ott_synth_version_info.get("version")
                    )
                )
            else:
        ...
        if phyle_v:  # ignore empty dict (no data found)
            ...
            if phyle_v.get("nominated_study_unique_OTU_count") is None:
                warnings.add(
                    "phylesystem.nominated_study_unique_OTU_count info not found!"
                )
            else:
                num_otu_in_nominated_studies = phyle_v.get(
        ...
        ott_version_released_today = (
            ott_new_version_info and ott_new_version_info.get("version", "") or str("")
        )
        ott_version_used_in_synth = ott_synth_version_info.get("version", "")
        by_date[date] = {
            "Unique OTUs in OTT": num_otu_in_ott,
...
@view_config(
    route_name="about_synthesis_release",
...
def synthesis_release(request):
    ...
    if synth_release_version == "":
        ...
        raise HTTPSeeOther(
            location="/about/synthesis-release/{v}".format(v=release_version)
        )
...
@view_config(
    route_name="about_taxonomy_version",
...
def taxonomy_version(request):
    ...
    if taxo_version == "":
        ...
        raise HTTPSeeOther(
            location="/about/taxonomy-version/{v}".format(v=taxonomy_version)
        )
    ...
    # fetch and render Markdown release notes as HTML
    fetch_url = "https://raw.githubusercontent.com/OpenTreeOfLife/reference-taxonomy/master/doc/{v}.md".format(
        v=taxo_version
    )
    try:
        version_notes_response = requests.get(url=fetch_url).text
