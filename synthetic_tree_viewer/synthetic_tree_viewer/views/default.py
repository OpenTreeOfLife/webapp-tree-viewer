...
@view_config(
    route_name="contact", renderer="synthetic_tree_viewer:templates/contact.jinja2"
)
@login_required
def contact(request):
...
@view_config(
    route_name="taxonomy_browser",
    renderer="synthetic_tree_viewer:templates/taxobrowse.jinja2",
)
...
def login(request):
    ...
    destination_url = (
        request.session.get("_next", None) and request.session.get("_next") or "/"
    )
    ...
    if login_result and login_result.user:
        ...
        log.debug(
            "TRUE login_result '%s' AND TRUE user '%s'", login_result, login_result.user
        )
        ...
        if recaster == "DEEP_COPY":
            ...
            # NO, this still creates an instance of the old/incompatible class!
        ...
        request.session["auth_user"] = {
            ...
            "github_profile_url": gh_user_data[
                "html_url"
            ],  # "https://github.com/jimallman"
        ...
        log.debug(
            "TRUE login_result '%s', BUT FALSE user '%s'  :-/",
            login_result,
            login_result.user,
        )
        ...
        log.debug(
            "FALSE login_result '%s'... probably jumping to OAuth now", login_result
        )
        # request.make_body_seekable()
        # raise RetryableException
        pass
...
def tree_view(request):
    ...
    if len(path_parts) > 0:
        if path_parts[0] in [
            ...
            "feedback",
            "properties",
        ]:  # TODO: add 'onezoom','phylet', others?
